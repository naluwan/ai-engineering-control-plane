...
its application use cases are in place. The Projects UI, the agent workflow and
the orchestration built on top of them are not.
...
5. **Projects API and application use cases** (TASK-005):
   ...
   - Endpoints `POST /api/projects`, `GET /api/projects` and
     `GET /api/projects/[id]`.
   - Zod strict validation at the HTTP boundary, so an unknown field is
   ...
   - Correlation identifiers, returned in `x-correlation-id` and recorded in
     the matching log line.
   - Credential-safe redaction, so no connection string, password or token can
     reach a log.
   - Dependency bootstrap performed inside the guarded HTTP boundary, so a
     failure to validate the environment or construct the client still produces
     the standard `INTERNAL_ERROR` response, correlation id and log.

6. **Environment and verification**. Validating configuration and opening a
   database connection are separate events, and they happen at different times:
   - **Startup validation.** `DATABASE_URL` is validated by the Node server
     startup instrumentation, which calls `loadAppEnv()`. A missing or
     malformed required variable therefore fails server startup, with a
     credential-free error message.
   - **Lazy persistence.** The Prisma client and the repositories it backs
     remain lazily initialised. Importing a module creates no database
     connection; startup validation checks the URL's shape without connecting
     to it.
   - **Guarded bootstrap.** Route dependency bootstrap happens inside the
     guarded HTTP boundary, so a runtime bootstrap failure is converted into
     the standard `INTERNAL_ERROR` response, with its correlation id and log,
     rather than escaping the handler.
   - **Build behaviour.** A production build opens no database connection.
     `pnpm build` does not require a reachable PostgreSQL instance.
...
1. **Projects UI** — creating, listing and viewing projects in the browser
   (TASK-006). `/projects` is still the placeholder route from TASK-003.
